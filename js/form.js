...
function validaPaciente(paciente) {
    ...
    //quando for um if que so verifica um argumento ele pode ser escrito assim para economizar linha 
    if (!validaPeso(paciente.peso)) erros.push("Peso é inválida");
    // porem essa é a forma convencional
    if (!validaAltura(paciente.altura)) {
        erros.push("Altura é inválida");
    }

    if (paciente.gordura.length == 0) {
        erros.push("A gordura do paciente não poder ser em branco")
    }
